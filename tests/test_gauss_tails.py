...
def test_pq():
    ...
    p = lambda x: jnp.where(x >= gt.mu, 1 / jnp.sqrt(2 * jnp.pi) * jnp.exp(-0.5 * x ** 2) / jstats.norm.cdf(-gt.mu),
                            0.0)
    q = lambda x: jnp.where(x >= gt.eta, 1 / jnp.sqrt(2 * jnp.pi) * jnp.exp(-0.5 * x ** 2) / jstats.norm.cdf(-gt.eta),
                            0.0)
...
def test_coupled_exponentials():
    ...
    np.testing.assert_allclose(x_mean, mu + 1 / alpha_mu, atol=1e-5, rtol=1e-5)
    np.testing.assert_allclose(y_mean, eta + 1 / alpha_eta, atol=1e-5, rtol=1e-5)
